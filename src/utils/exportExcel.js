// 导出LuckySheet使用exceljs实现
const Excel = require('exceljs')

/**
 * 返回一个 buffer对象,可以使用 创建一个blob对象下载，外部调用需要 await等待完成关键字，因为这个方法是异步的<br>
 * const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
 * @param {*} luckysheet 参数为luckysheet.getluckysheetfile()获取的对象
 * @returns 
 */
exports.exportExcel = async function (luckysheet) { // 参数为luckysheet.getluckysheetfile()获取的对象
    ...
    luckysheet.every(function (table) {
        ...
        // 等待上面数据写入之后，这里可以设置单元格宽度和行的高度
        // setColumnlen(table.config.columnlen, worksheet)
        // 设置所有列的默认宽度
...
var setColumnlen = function (luckyColumnlen, worksheet) {
    // 将LuckySheet的 config.columnlen 对象解析为数组，方便循环
    let cArray = Object.values(luckyColumnlen)
    for (let i = 0; i < worksheet.columns.length; i++) {
        worksheet.columns[i].width = cArray[i]
    }
}
