<!--  -->
<template>
  <div class="sn-container-publish">
    <van-nav-bar :title="$t(`dingding.publishShowTitle`)" :left-text="$t(`dingding.back`)" @click-left="onClickLeft" />
    ...
    <div class="form" style="margin:1rem">
      <el-form label-position="left" label-width="9rem" :model="form" :rules="rules">
        ...
        <el-form-item :label="$t(`dingding.message`)" prop="message">
          <el-input v-model="form.message" type="textarea" :rows="3" placeholder="Please enter the message."></el-input>
        </el-form-item>
...
import { publishShowNew, shows, members } from '@/api/dingding'
export default {
  ...
  methods: {
    async submit() {
      ...
      const { data: res } = await publishShowNew(this.form)
      if (res.code == 200) {
        this.$message({
          type: 'success',
          message: '发布成功'
