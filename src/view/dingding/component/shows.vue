...
<template>
    <div class="shows-container">
        <!-- <div style="font-size: 1.5rem;padding: 1rem;color: #888;">展会消息列表</div>
        <div class="change-lan" style="margin:1rem;position:absolute;top: 0;right:0rem;">
            <ChangeLan></ChangeLan>
        </div> -->
        <van-nav-bar :title="$t(`dingding.extList`)" />
        <div class="change-lan" style="margin:0.7rem;position:absolute;top: 0;right:0rem;z-index: 99;">
            <ChangeLan></ChangeLan>
        </div>
        <van-swipe-cell v-for="item in shows" :key="item.showId" style="margin:1rem 0;background:#fff;">
            ...
            <template #right>
                ...
                <div>
                    <van-button square hairline size="large" type="danger" @click="deleteById(item.showId)"
        ...
        <van-action-sheet v-model="show" :title="$t(`dingding.update`)">
            <van-form @submit="onSubmit">
                ...
                <van-field v-else v-model="updateForm.memberName" :label="$t(`dingding.publisher`)"
                    :placeholder="$t(`dingding.publisher`)" readonly label-width="7rem" />
...
import { shows, deleteShowById, updateShow } from '@/api/dingding'
export default {
    ...
    methods: {
        ...
        async deleteById(id) {
            console.log(id)
            const { data: res } = await deleteShowById(id)
            if (res.code == 200) {
                this.getShows()
                this.$message({
                    type: 'success',
        ...
        async onSubmit() {
            ...
            const { data: res } = await updateShow(this.updateForm)
            if (res.code == 200) {
                this.$message({
                    type: 'success',
                    message: '更新成功'
