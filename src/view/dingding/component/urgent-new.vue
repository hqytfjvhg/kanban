<!--  -->
<template>
    <div class="un-container-publish">
        <van-nav-bar :title="$t(`dingding.publishNoticeTitle`)" :left-text="$t(`dingding.back`)"
        ...
        <div class="form" style="margin:1rem">
            <el-form label-position="left" label-width="9rem" :model="form" :rules="rules">
                <el-form-item :label="$t(`dingding.title`)" prop="title">
                    <el-input v-model="form.title"></el-input>
                </el-form-item>
                <el-form-item :label="$t(`dingding.module`)" prop="screenId">
                    <el-select v-model="form.screenId" placeholder="Please select a module">
            ...
            <div style="display:flex">
                ...
                <el-button type="warning" style="margin:1rem auto" @click="queryInfos()">{{ $t(`dingding.queryList`)
                }}</el-button>
            </div>
        </div>
        <van-tabbar route>
            ...
            <van-tabbar-item icon="friends-o" :to="{ name: 'shows-publish' }">{{ $t(`dingding.tabTitle2`) }}
            </van-tabbar-item>
            <van-tabbar-item icon="setting-o" :to="{ name: 'notices-publish' }">{{ $t(`dingding.tabTitle3`) }}
            </van-tabbar-item>
        </van-tabbar>
    </div>
</template>
...
import { publishUrgentNotice, members, notices } from '@/api/dingding'
export default {
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                screenId: [
                    { required: true, message: this.$t(`dingding.moduleRuleTip`), trigger: 'blur' },
                ],
            },
            users: {},
            options: [{
    ...
    methods: {
        async submit() {
            ...
            const { data: res } = await publishUrgentNotice(this.form)
            console.log(res)
            if (res.code == 200) {
                this.$message({
                    type: 'success',
