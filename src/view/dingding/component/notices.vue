...
<template>
  <div class="notices-container">
    <!-- <div style="font-size: 1.5rem;padding: 1rem;color: #888;">紧急通知列表</div> -->
    <van-nav-bar :title="$t(`dingding.noticeList`)"/>
    <div class="change-lan" style="margin:0.7rem;position:absolute;top: 0;right:0rem;z-index: 99;">
      <ChangeLan></ChangeLan>
    </div>
    <van-swipe-cell v-for="item in notices" :key="item.urgentNoticeId" style="margin:1rem 0;background:#fff;">
      ...
      <template #right>
        <div>
          <van-button square hairline size="large" type="primary" :text="$t(`dingding.update`)" @click="update(item)" />
        </div>
        <div>
          <van-button square hairline size="large" type="danger" @click="deleteById(item.urgentNoticeId)"
    ...
    <van-action-sheet v-model="show" :title="$t(`dingding.update`)">
      <van-form @submit="onSubmit">
        <van-field v-model="updateForm.title" :name="$t(`dingding.title`)" :label="$t(`dingding.title`)"
          :placeholder="$t(`dingding.title`)" :rules="[{ required: true, message: 'Please enter the message' }]"
          label-width="7rem" />
        <van-field name="radio" :label="$t(`dingding.module`)">
          <template #input>
            <van-radio-group v-model="updateForm.screenId" direction="horizontal">
              <van-radio :name="2">Marketing</van-radio>
              <van-radio :name="3">STS</van-radio>
            </van-radio-group>
          </template>
        </van-field>
        <van-field v-if="updateForm.memberName === null" v-model="updateForm.publisher" :name="$t(`dingding.publisher`)"
        ...
        <van-field v-else v-model="updateForm.memberName" :name="$t(`dingding.publisher`)"
          :label="$t(`dingding.publisher`)" :placeholder="$t(`dingding.publisher`)" readonly label-width="7rem" />
        <van-field v-model="updateForm.message" :label="$t(`dingding.message`)" :placeholder="$t(`dingding.message`)"
          autosize :rules="[{ required: true, message: 'Please enter the message' }]" label-width="7rem" />
        <van-field readonly clickable name="datetimePicker" :value="updateForm.expireTime"
          :label="$t(`dingding.expiration`)" placeholder="Please select the datetime" @click="showPicker = true"
          label-width="7rem" />
        <van-field name="switch" :label="$t(`dingding.isEffective`)" label-width="7rem">
          <template #input>
            <van-switch v-model="updateForm.isExpire" size="20" inactive-color="#13ce66" active-color="#ff4949" />
          </template>
        </van-field>
...
import { notices, deleteNoticeById, updateUrgentNotice } from '@/api/dingding'
export default {
  ...
  methods: {
    ...
    async deleteById(id) {
      console.log(id)
      const { data: res } = await deleteNoticeById(id)
      if (res.code == 200) {
        this.getNotices()
        this.$message({
          type: 'success',
    ...
    async onSubmit() {
      ...
      const { data: res } = await updateUrgentNotice(this.updateForm)
      if (res.code == 200) {
        this.$message({
          type: 'success',
          message: '更新成功'
