    ...
    <div class="ticket-bottom">
      <div style="width: 93%; border-radius: 2px; padding-top: 20px">
        <div class="ticket-box">
          ...
          <div class="ticket-content">
            <div class="ticket-item">
              <img :src="src3" alt="" @click="changeOne(3)" />
              <el-checkbox
...
<script>
import { toTicket, getTeamId, getStoId } from "@/api/act";
export default {
  data() {
    return {
      active: 1,
  ...
  methods: {
      ...
      getTeamId().then((res) => {
        if (res.code == 200) {
          ...
          for (let a = 0; a < 5; a++) {
            if (data[a] == 1) {
              if (!that.title1) {
                that.title1 = that.firstLabel[a];
                that.first1 = a;
                that.src1 =
                  "https://linyi-oss.oss-cn-shenzhen.aliyuncs.com/" +
                  (a + 1) +
                  ".jpg";
              } else if (!that.title2) {
                that.title2 = that.firstLabel[a];
                that.first2 = a;
                that.src2 =
                  "https://linyi-oss.oss-cn-shenzhen.aliyuncs.com/" +
                  (a + 1) +
    ...
    confirmResult() {
      ...
      } else if (this.checkeds[1].checked) {
        for (let b = 0; b < 5; b++) {
          if (b == that.first2) {
            data.push(1);
          } else {
      ...
      } else if (this.checkeds[2].checked) {
        for (let b = 0; b < 5; b++) {
          if (b == that.first2) {
            data.push(1);
          } else {
