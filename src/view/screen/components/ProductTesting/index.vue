...
<template>
  <div class="ProductTesting">
    <div class="ticket-box" id="ticket-box" :style="'height:' + height">
      <div class="ticket-title" id="ticket-title">Product Testing Stage</div>
      <el-table class="table-style" :data="data" :height="280" size="mini" ref="stage" :cell-style="cellStyle"
        ...
        <el-table-column align="center" label="Product" prop="product" />
        <el-table-column align="center" label="Eng Sample testing" prop="engSample" :formatter="formatTime" width="185px" />
        <el-table-column align="center" label="PP Testing" prop="pp" :formatter="formatTime" width="160px" />
        <el-table-column align="center" label="MP" prop="mp" :formatter="formatTime" width="160px" />
        <el-table-column align="center" label="Launch" prop="launch" :formatter="formatTime" width="160px" />
      </el-table>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    headerCellStyle({ columnIndex }) {
      if (columnIndex == 2) {
        ...
        return "font-size: 1rem;font-weight: 600;background: #0f0938; white-space:nowrap; border: none; border-right: 1px solid #333; padding: 0; height:" + this.ticketTableCellHeight + 'px; color: white;'
      }
    },
    headerRowStyle() {
    ...
    formatTime(row, column) {
      ...
      // console.log(column)
      if (column.label === "Eng Sample testing") {
        if (row.engSample == null) return "/"
        const t1 = new Date(row.engSample)
        const date1 = t1.getDate().toString()
        const month1 = t1.getMonth()
        const year1 = t1.getFullYear() - 2000
        return date1 + '-' + months[month1] + '-' + year1
      }
      if (column.label === "PP Testing") {
