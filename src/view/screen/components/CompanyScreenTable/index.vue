<!-- 2023-12-23 新增表格，当前页面目前还是固定静态数据没有接入后台，用于 Company 大屏页面使用  -->
<template>
  <div class="ProductStage">
    <div class="ticket-box" id="ticket-box" :style="'height:' + height">
      <div class="ticket-title" id="ticket-title">News</div>
      <!-- <el-table
        class="table-style"
        :data="stages"
        height="100"
      ...
      <div
        style="width: 100%; height: 86%; overflow: hidden; padding-bottom: 1rem"
        id="box_content"
      >
        <div v-for="(item, i) in stages" :key="i" class="div_box">
          <el-card class="card_box">
            <template #header>
              <div class="card-header">
                <span>{{ item.name }}</span>
              </div>
              <div style="margin-top: 0.8rem" class="date-style">
                {{ formatDate(item.productDate) }}
              </div>
            </template>
...
export default {
  ...
  methods: {
    ...
    tableScroll(table, speed) {
      // 拿到表格挂载后的真实DOM
      const divData = document.getElementById("box_content");
      setInterval(() => {
