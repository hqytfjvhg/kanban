...
<template>
  <div class="unsolvedTicket">
    <div class="ticket-box" id="ticket-box" :style="'height:' + height">
      <div class="ticket-title" id="ticket-title">Unsolved Tickets</div>
      <el-table class="table-style" :data="unsolvedComplaints" ref="unsolvedTicket" :height="tableHeight" :cell-style="cellStyle"
        :header-cell-style="headerCellStyle" :header-row-style="headerRowStyle">
        <el-table-column align="center" label="Time" prop="time">
        </el-table-column>
        <el-table-column align="center" label="Row" prop="rowNum" :width="Math.ceil(5 * rem)">
        </el-table-column>
        <el-table-column align="center" label="China" prop="chinaNum">
        </el-table-column>
      </el-table>
    </div>
  </div>
...
import { getUnsolvedTicket } from '@/api/sts';
export default {
  ...
  methods: {
    async getUnsolvedTickets() {
      //getUnsolvedTicket
      const { data: res } = await getUnsolvedTicket()
      if (res.code == 200) {
        this.data = res.data
      }
    },
    cellStyle({ rowIndex, columnIndex }) {
      ...
      if (rowIndex % 2 == 0 && columnIndex == 0) {
        ...
        return 'font-size: 0.9rem; background: #15184d; border: none; padding: 0; height: ' + this.ticketTableCellHeight + 'px'
