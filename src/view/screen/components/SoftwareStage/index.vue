<!--  -->
<template>
  <div class="SoftwareStage">
    <div class="ticket-box" id="ticket-box" :style="'height:' + height">
      <div class="ticket-title" id="ticket-title">Software/Firmware</div>
      <el-table class="table-style" :data="SoftwareStages" :height="'600px'" size="mini" ref="stage"
        :cell-style="cellStyle" :header-cell-style="headerCellStyle" :header-row-style="headerRowStyle">
        <el-table-column align="center" label="product" prop="product" />
        <el-table-column align="center" label="Version" prop="version" />
        <el-table-column align="center" label="PRC" prop="prc" :formatter="formatTime" width="160px" />
        <el-table-column align="center" label="UK" prop="uk" :formatter="formatTime" width="160px" />
        <el-table-column align="center" label="Launch" prop="launch" :formatter="formatTime" width="160px" />
        <el-table-column align="center" label="Remark" prop="remark" :formatter="formatTime" width="160px" />
      </el-table>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    formatTime(row, column) {
      ...
      // console.log(column)
      if (column.label === "Version") {
        if (row.version == null) return "/"
        const t1 = new Date(row.version)
      ...
      if (column.label === "PRC") {
        if (row.prc == null) return "/"
        const t1 = new Date(row.prc)
        const date1 = t1.getDate().toString()
        const month1 = t1.getMonth()
        const year1 = t1.getFullYear() - 2000
        return date1 + '-' + months[month1] + '-' + year1
      ...
      if (column.label === "UK") {
        if (row.uk == null) return "/"
        const t1 = new Date(row.uk)
        const date1 = t1.getDate().toString()
        const month1 = t1.getMonth()
        const year1 = t1.getFullYear() - 2000
        return date1 + '-' + months[month1] + '-' + year1
      ...
      if (column.label === "Remark") {
        if (row.remark == null) return "/"
        const t1 = new Date(row.remark)
        const date1 = t1.getDate().toString()
        const month1 = t1.getMonth()
        const year1 = t1.getFullYear() - 2000
        return date1 + '-' + months[month1] + '-' + year1
