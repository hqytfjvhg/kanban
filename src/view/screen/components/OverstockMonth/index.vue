<!--  -->
<template>
  <div class="OverstockMonth">
    <div class="ticket-box" id="ticket-box" :style="'height:' + height">
      <div class="ticket-title" id="ticket-title">
        Overstock
        <span v-if="type==1">(Order By Quantity)</span>
        <span v-if="type==2">(Order By $)</span>
      </div>
      <el-table class="table-style" :data="overstocks" ref="unsolvedTicket" :height="'380'" :cell-style="cellStyle"
        ...
        <el-table-column align="center" :label="months[(monthIndex - 2 + 12) % 12]" prop="num1">
          <template slot-scope="scope">
            {{ scope.row.num1 == null ? '-' : scope.row.num1 }}
          </template>
        </el-table-column>
        <el-table-column align="center" :label="months[(monthIndex - 1 + 12) % 12]" prop="num2">
          <template slot-scope="scope">
            {{ scope.row.num2 == null ? '-' : scope.row.num2 }}
          </template>
        </el-table-column>
        <el-table-column align="center" :label="months[(monthIndex + 12) % 12]" prop="num3">
          <template slot-scope="scope">
            {{ scope.row.num3 == null ? '-' : scope.row.num3 }}
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    cellStyle({ rowIndex, columnIndex }) {
      ...
      //console.log(`height number`,Number(strArray[0]))
      //console.log(Math.ceil(Number(strArray[0])/7.8))
      if (rowIndex % 2 == 0 && columnIndex == 0) {
        ...
        return 'font-size: 1rem; background: #15184d;font-weight: 800; border: none; padding: 0; height: ' + this.ticketTableCellHeight + 'px'
